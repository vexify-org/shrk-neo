...
class SharedMemory {
  ...
  /**
   * Lightweight usage stats. `bytesUsed` is the allocator's high-water mark
   * since the last clear() (deleted keys' bytes are reclaimed by clear()).
   */
  stats () {
    let liveKeys = 0
    for (let i = 0; i < this.slots; i++) {
      if (Atomics.load(this.ints, slotBase(i) + SLOT_STATE) === S_READY) liveKeys++
    }
    return {
      slots: this.slots,
      liveKeys,
      freeSlots: this.slots - liveKeys,
      bytesUsed: Atomics.load(this.ints, IDX_CURSOR),
      bytesCapacity: this.sab.byteLength - this.dataStart
    }
  }
